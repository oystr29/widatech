...
import { invoices as invoicesTable, invoicesToProducts as invoicesToProductsTable, } from "../db/schema.js";
...
invoices.get("/:id", async (c) => {
    ...
    try {
        const query = await db.execute(sql.raw(`SELECT 
    i.id, 
    i.invoice_no, 
    i.date, 
    i.customer_name, 
...
LIMIT 1;`));
        ...
        const invoicesToProducts = JSON.parse(data.invoicesToProducts);
        const summary = {
            total: invoicesToProducts.reduce((a, b) => a + b.qty * b.product_price, 0),
        };
        return c.json({
            data: {
                date: data.date,
...
invoices.get("/:id/products", async (c) => {
    ...
    const summary = {
        total: invoice?.invoicesToProducts.reduce((a, b) => a + b.qty * b.product_price, 0),
    };
    return c.json({ data: { ...invoice, summary } });
});
invoices.post("/", zValidator("json", z.object({
    date: z.iso.date(),
    ...
    try {
        ...
        const dateTime = new Date(date.getFullYear(), date.getMonth(), date.getHours(), date.getMinutes(), date.getSeconds(), date.getMilliseconds()).getTime();
